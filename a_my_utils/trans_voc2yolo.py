...
voc_root = r"/data1/wanglu/datasets/insulator_defect/defect2twoclass/"                                #VOC root
voc_version = "VOC2007"
...
save_file_root = r"/data1/wanglu/datasets/insulator_defect/defect2twoclass/v5"
# label标签对应json文件
label_json_path = r'/home/wanglu/yolov5_6.2_1/a_my_utils/pascal_voc_classes.json'
...
train_txt_path = os.path.join(voc_root, voc_version, "ImagesSets", "main", train_txt)
val_txt_path = os.path.join(voc_root, voc_version, "ImagesSets", "main", val_txt)
    ...
    for file in tqdm(file_names, desc="translate {} file...".format(train_val)):
        ...
        if not os.path.exists(img_path):
            ...
            if not os.path.exists(img_path):
                img_path = os.path.join(voc_images_path, file + ".JPG")
                if not os.path.exists(img_path):
                    img_path = os.path.join(voc_images_path, file + ".jpeg")
        ...
        # read xml
        with open(xml_path,encoding='gbk') as fid:
            xml_str = fid.read()
        ...
        with open(os.path.join(save_txt_path, file + ".txt"), "w",encoding='gbk') as f:
            print(os.path.join(save_txt_path, file + ".txt"))
            for index, obj in enumerate(data["object"]):
                ...
                class_name = obj["name"]
                #if str(class_name) == 'plate, tower, insulator, spacer, damper':
                #if str(class_name) == 'plate':
                 # continue
                # print(class_name)
...
def create_class_names(class_dict: dict):
    ...
    with open("./data/my_data_label.names", "w",encoding='gbk') as w:
...
def main():
    # read class_indict
    json_file = open(label_json_path, 'r',encoding='gbk')
    class_dict = json.load(json_file)

    # 读取train.txt中的所有行信息，删除空行
    with open(train_txt_path, "r",encoding='gbk') as r:
        train_file_names = [i for i in r.read().splitlines() if len(i.strip()) > 0]
    # voc信息转yolo，并将图像文件复制到相应文件夹
    translate_info(train_file_names, save_file_root, class_dict, "train")

    # 读取val.txt中的所有行信息，删除空行
    with open(val_txt_path, "r",encoding='gbk') as r:
